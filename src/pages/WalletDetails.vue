<template>
    <div>
        <div class="d-flex align-items-center mb-2">
            ...
            <img v-if="walletData.imageUrl !== undefined" class="ms-2" style="width: 40px; height: 40px" v-bind:src="'https://www.cryptocompare.com' + walletData.ImageUrl" />
        </div>
        <p class="card-text">
            ...
            <span v-if="walletData.Rating" class="d-block">Оценка адаптивности к различным алгоритмам: {{ walletData.Rating.Weiss.TechnologyAdoptionRating }}</span>
        </p>
        <div class="d-flex align-items-center">
            <h4 class="me-3">Показать изменение курса валюты за последние:</h4>
            <input @input="(event) => validateGraphDays(event)" v-model.number="graphDays" type="text" class="form-control form-control-md w-20 me-2" placeholder="Введите количество дней" @keydown.enter="updateGraphData(this.graphDays)" />
            <div class="btn btn-success" @click="updateGraphData(this.graphDays)">Показать</div>
        </div>
        <Wallet-graph :graphValues="this.graphData" @clearSelectedTicker="this.selectedTicker = null" :btnVisible="false" :graphSelWidth="'40px'" :title="this.currentLimit" />
    </div>
</template>
<script>
